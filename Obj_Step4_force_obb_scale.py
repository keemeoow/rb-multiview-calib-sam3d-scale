#!/usr/bin/env python3
"""
./obj{N}_sam3d.glb 의 AABB 를 측정된 OBB extents 에 강제 맞춤.

uniform scale 만으로는 sam3d 메시 aspect ratio 와 실제 물체가 달라서
1축만 일치하는 문제가 있음 → vertex 에 축별 비균등 scale 을 적용해
GLB AABB 가 실측 OBB extents 와 (rank-매칭으로) 일치하도록 보정.

[실행]
python3 Obj_Step4_force_obb_scale.py \
  --glb_root . \
  --size_json ./size_measurements.json \
  --suffix _obb

→ ./obj{N}_sam3d_obb.glb 생성. (원본 보존)
"""
...
def main():
    ...
    ap.add_argument("--glb_root", default=".",
                    help="obj{N}_sam3d.glb 들이 있는 루트 (기본: 현재 디렉토리)")
    ap.add_argument("--size_json", default="./size_measurements.json")
    ap.add_argument("--suffix", default="_obb",
                    help="새 GLB 파일명 suffix (예: '_obb' → obj1_sam3d_obb.glb). "
                         "빈 문자열이면 원본 덮어쓰기.")
    args = ap.parse_args()
    ...
    for glb_in in sorted(root.glob("obj*_sam3d.glb")):
        name = glb_in.stem.replace("_sam3d", "")
        ...
        glb_out = root / f"{name}_sam3d{args.suffix}.glb"
