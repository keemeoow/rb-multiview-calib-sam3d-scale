#!/usr/bin/env python3
"""
Obj_Step3c_fit_cad_silhouette.py

CAD (또는 형상만 아는 메시) 가 있을 때의 **최선의 크기 추정**.
...
[실행]
  python Obj_Step3c_fit_cad_silhouette.py \
...
from _silhouette_fit import (
    View, clean_cloud, cloud_from_masked_depth, fit_cad_to_views,
...
def build_cloud(capture_dir: Path, mask_dir: Path, obj: str, cams, depth_scale: float,
    ...
    return clean_cloud(np.vstack(parts))
...
def main():
    ...
    ap.add_argument("--init_erode_px", type=int, default=3,
                    help="초기 점군을 만들 때 마스크 침식 (실루엣 경계의 mixed-pixel 제거)")
    ap.add_argument("--w_depth", type=float, default=0.0,
                    help="손실에 더할 depth 잔차 가중치(m 단위). 검은/광택 물체에서는 0 권장.")
    ...
    print(f"[INFO] w_depth={args.w_depth} ({'pure silhouette' if args.w_depth == 0 else 'joint'})")
    ...
    for obj, cad_path in cad.items():
        ...
        cloud = build_cloud(args.capture_dir, args.mask_dir, obj, cams,
                            args.depth_scale, args.init_erode_px)
        print(f"  init cloud: {len(cloud)} points")

        views, cam_meta = build_views(args.capture_dir, args.mask_dir, obj, cams)
        fit = fit_cad_to_views(mesh, cloud, views, w_depth=args.w_depth, max_fev=args.max_fev)
        ...
        info = {
            "obj": obj,
            "cad": str(cad_path),
            "method": "cad_multiview_silhouette" if args.w_depth == 0 else "cad_silhouette_plus_depth",
